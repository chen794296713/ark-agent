...
A built-in **Mock Agent Manager** (`lib/agent-manager/mock.ts`) stands in for the real service whenever `AGENT_MANAGER_MODE` ≠ `live` (the default). It runs **in-process and synchronously**: `provisionAgent` returns VM ids and a `working` status immediately, and `sendMessage` returns a role-flavored reply inline — so the full hire → provision → chat flow (US3, US6) is exercisable with no external dependency or webhook loop. The HMAC webhook endpoint still exists for `live` mode, where the real Agent Manager drives `provisioning → deploying → working` and streams activity / heartbeats / inbound messages back. The active client is selected by `getAgentManager()` (`lib/agent-manager/index.ts`).
...
| Payments | **Stripe** (global) / **Alipay** (China) | Region-appropriate processors behind one billing model (US9). *Checkout is currently simulated — `POST /api/billing/checkout` records a paid invoice + subscription; real SDK wiring is a follow-up.* |
...
**Login identifier.** `POST /api/auth/login` accepts an **email *or* a plain username** (lowercased, looked up against `users.email`) — so the seeded `demo` account signs in as `demo` / `demo123`. **Registration still requires a real email** (`z.string().email()`), so every account a user creates is keyed to a real address and starts on real, empty data.

**Route protection.**
- **API route handlers** (`app/api/**`) are the authoritative authorization boundary: each calls `getAuthContext()` first, returns `401` when unauthenticated, and filters every subsequent query by `workspace.id` for tenant isolation (§9). This is the real check — it hits the DB.
- **The dashboard shell** (`app/dashboard/layout.tsx`, a client component) gates the UI: it loads the session via `GET /api/auth/me`, shows a loading state until `authReady`, and `router.replace("/auth")` when there is no user. Because every protected page fetches its data from the workspace-scoped API, an unauthenticated client simply receives `401`s — the server never returns another tenant's data regardless of the client-side redirect. (There is no Middleware/Proxy file; the API is the single boundary.)
...
`agents` is the central record: workspace + creator, `roleId`, `engine` (`openclaw`/`hermes`), `planTier`, the operator-authored `instructions` and `rules` (the "job brief", US3), a credit counter, a 9-value `status` (`draft`, `provisioning`, `deploying`, `working`, `scheduled`, `needs_review`, `paused`, `error`, `terminated`), and a **`settings` JSONB** (engine-aware configuration, see below). It also carries the **fields shared with the Agent Manager**: `agentManagerId` (unique), `vmId`, `vmRegion`, `deploymentStatus`, `lastError`, `lastHeartbeatAt`, `provisionedAt`, `uptimeStartedAt`. The four child tables back the agent-detail tabs (US5): `agent_tasks` (queued/in_progress/done/blocked), `agent_activities` (the live feed, US4, tagged via `activity_tag`), `agent_metrics` (the Performance tab — label/value/delta/weighted bar), and `agent_improvements` (the **self-review queue**: `pending`/`approved`/`dismissed`).

**Agent settings** (`lib/agent-settings.ts`; column added in migration `0002`). `agents.settings` JSONB holds the rich, engine-aware configuration surfaced in the Settings tab (US5): behavior (tone, reply language, timezone), autonomy & approvals (`suggest`/`ask`/`auto`, a `$` approval threshold, approve-external-sends, daily action limit), schedule (always-on vs working hours/days + heartbeat cadence), model & reasoning (model-agnostic picker, temperature, reasoning depth), **OpenClaw** skills + local-execution tool permissions (shell/files/browser/docker/code), the **Hermes** self-improving learning loop, memory & knowledge sources, and a monthly credit cap. A typed `AgentSettings` model with `DEFAULT_SETTINGS` is **merged over defaults on read** (`mergeSettings`), so legacy rows and partial updates always resolve to a complete object. (OpenClaw and Hermes are real open-source agent runtimes; the settings mirror their actual capabilities — OpenClaw's Plugins/Skills + Local Execution, Hermes's model-agnostic provider + learning loop.)
...
| Group | Endpoints (as built) | Stories |
| --- | --- | --- |
...
| **Webhooks** | `POST /api/webhooks/agent-manager` (single endpoint; typed event; HMAC-verified, **no** session cookie) | §6 |

Convention: mutations return the updated resource (e.g. `{ agent }`); list endpoints return a named array (e.g. `{ agents: [...] }`, `{ roles: [...] }`). Validation failures → `422` (with Zod issue details), auth failures → `401`, cross-workspace access → `404` (not `403`, to avoid leaking existence). Every handler declares `runtime = "nodejs"` + `dynamic = "force-dynamic"`, so control-plane reads stay request-time and operators always see live state.
...
**Outbound (ArkAgent → Agent Manager).** `lib/agent-manager/live.ts` calls the base URL `AGENT_MANAGER_BASE_URL`, authenticated with `Authorization: Bearer ${AGENT_MANAGER_API_KEY}` (JSON bodies, `cache: "no-store"`).

| Call | Trigger | Effect |
| --- | --- | --- |
| `POST /v1/agents` (provision + deploy) | Launch in hire wizard (US3) | Creates the VM + installs the chosen engine, returns `{ agentManagerId, vmId, vmRegion, status, deploymentStatus }` which ArkAgent stores on the agent |
| `PATCH /v1/agents/{amId}` (sync config) | Edit instructions/rules/channels (US5) | Pushes the new brief / channel attachments to the running agent |
| `POST /v1/agents/{amId}/messages` (relay) | User sends a web-chat message (US6) | Delivers the outbound message; in live mode the reply returns later via the `agent.message` webhook |
| `POST /v1/agents/{amId}/lifecycle` — `{ action }` | Lifecycle controls (US8) | Pauses / resumes / terminates the runtime; ArkAgent reflects the returned `status` |

**Inbound (Agent Manager → ArkAgent webhooks).** All land at the single endpoint `POST /api/webhooks/agent-manager`, carry no session cookie, and are HMAC-verified before any write. The body is a typed `WebhookEvent` discriminated on `type`, always carrying `externalAgentId` (= ArkAgent's `agents.id`):

| `type` | Updates |
| --- | --- |
| `agent.status` | Advances `agents.status` + `vmId`/`vmRegion`/`deploymentStatus` (drives `provisioning → deploying → working`, US3); sets `lastError` on failure |
| `agent.heartbeat` | Stamps `lastHeartbeatAt` (+ `uptimeStartedAt`) — dashboard liveness (US4) |
| `agent.message` | Inserts an `agent`-sender `messages` row (deduped on unique `externalId`) into the latest conversation, creating one if needed (US6) |
| `agent.activity` | Appends an `agent_activities` row (live feed, US4) |
| `agent.metric` | Inserts an `agent_metrics` row (Performance tab, US5) |
| `agent.improvement` | Enqueues an `agent_improvements` row as `pending` (self-review queue, US5) |
| `agent.usage` | Appends a `usage_records` ledger row and **atomically** increments `workspaces.creditsUsed` + `agents.creditsUsed` (US9) |

**HMAC verification (inbound).** The shared secret `AGENT_MANAGER_WEBHOOK_SECRET` keys an HMAC-SHA-256 over the **raw request body**; the hex digest travels in the `x-arkagent-signature` header (optional `sha256=` prefix). `lib/agent-manager/webhook.ts` recomputes over the raw text (read *before* JSON-parsing so the bytes match exactly) and compares with `timingSafeEqual` — a mismatch returns `401` with no side effect. Inbound writes are **idempotent** (the unique `externalId` dedupes message retries; credit increments are atomic), since at-least-once delivery means retries are expected. (Outbound calls are Bearer-authenticated only; signing applies to the inbound direction.)
...
**Rendering model.** The marketing/landing, auth, and the entire dashboard are **client components** that fetch from the workspace-scoped Route Handlers through a typed client (`lib/client-api.ts`); Drizzle and `getAuthContext()` run inside the API layer (server-side, Node runtime), which is the single authorization boundary. This suits the highly interactive console — the hire wizard, live chat, and the multi-section Settings tab each own substantial client state — while keeping all DB access behind the API. The React Compiler (`reactCompiler: true`) handles memoization; `lib/auth.ts`, `lib/db`, and the Agent Manager client are `server-only` and can never be bundled to the browser.
...
**Database connection.** `lib/db/index.ts` parses `DATABASE_URL` manually rather than handing the raw URL to `postgres-js`, because the pooler URL carries non-libpq params (`pgbouncer`, `connection_limit`, `pool_timeout`). When `pgbouncer=true`, **prepared statements are disabled** (`prepare: false`) since transaction-pooling mode is incompatible with them. The Drizzle client is created **lazily on first query** (behind a `Proxy`) and memoized on `globalThis`, so importing `lib/db` is side-effect-free — `next build`, which imports route modules to read their config, never fails when `DATABASE_URL` is absent at build time — while still surviving HMR and avoiding pool exhaustion in dev. DDL/migrations use the **direct** (non-pooled) `DIRECT_DATABASE_URL`.
...
**Channel secret handling (US7).** `channels.config` is JSONB holding connection credentials (bot tokens, API keys, per-channel secrets). Secrets are **never returned to the client**: `serializeChannel` masks any value whose key matches `/token|secret|key|password/i` to `••••••••` before responding, and secrets are never logged. **Hardening note:** secrets are currently persisted as-is in the JSONB and masked only on read; encrypting them **at rest** (AES-256-GCM keyed by a server-side `CHANNEL_ENCRYPTION_KEY`) is a planned hardening, not yet wired.
...
| `SESSION_SECRET` | 32-byte hex secret reserved for session signing |
...
*Planned, not yet wired: `CHANNEL_ENCRYPTION_KEY` (at-rest channel-secret encryption, §9) and real `STRIPE_*` / `ALIPAY_*` keys (checkout is simulated, US9).* `NODE_ENV` (set by Vercel) toggles `secure` cookies and the dev DB-client memoization.
...
**Migrations.** SQL migrations (`0000`–`0002`) are versioned in `lib/db/migrations/` (generated by `npm run db:generate`) and applied with `npm run db:migrate` against `DIRECT_DATABASE_URL` as a release step (not during the serverless request path). `npm run db:seed` loads the reference data (8 `agent_roles`, 3 `plans`) and the single mock-data **`demo` / `demo123`** account (it also cleans up legacy demo logins); every other user registers into an empty workspace.

**Regions & environments.** Global traffic runs on `arkagent.ai` (live); the China deployment targets `iagent.cc` with region-appropriate Postgres, the Alipay provider, and locale defaulting toward `zh`/`zht`. Deployments default to the **Mock Agent Manager** (`AGENT_MANAGER_MODE=mock`, no real VMs) so the full US3–US8 lifecycle is exercisable safely; setting `AGENT_MANAGER_MODE=live` + `AGENT_MANAGER_BASE_URL` switches to the real service.
